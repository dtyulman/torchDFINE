...
## model
_config.model = CN() 
_config.model.hidden_layer_list = [32,32,32] # Hidden layer list where each element is the number of neurons for that hidden layer of DFINE encoder/decoder. Please use [20,20,20,20] for nonlinear manifold simulations.
_config.model.activation = 'tanh' # Activation function used in encoder and decoder layers
_config.model.dim_y = 30 # Dimensionality of neural observations
_config.model.dim_a = 16 # Dimensionality of manifold latent factor, a choice higher than dim_y (above) may lead to overfitting
...
_config.model.save_dir = os.path.join(os.getcwd(), 'results', 'train_logs', str(datetime.now().strftime('%Y-%m-%d-%H%M%S'))) # Main save directory for DFINE results, plots and checkpoints
