...
# load_rnn = r"C:\Users\bilgi\Downloads\RNN_x=32_s=2_u=32_y=2.pt"
...
config.model.dim_a = args.dim_x
config.model.dim_y = 2*dim_y if train_dfine_on=='both' else dim_y
config.model.hidden_layer_list = None if config.model.no_manifold else [20,20]#[30,30,30,30]
config.model.activation = None if config.model.no_manifold else 'tanh'
config.model.save_dir = config.model.save_dir + ('_rnn'
                                                 f"_u={'_'.join([str(v) for v in train_input.values()])}") + no_manifold_key + f'_seed_{seed}' + f'_dim_x{dim_x}'
#training parameters
...
path_mse = []
final_point_mse = []
R_list = torch.arange(.1,30,.1).tolist()
...
R = R_list[torch.argmin(path_mse)]
print(f'R = {R}')
print(f'path mse = {min(path_mse)}')
print(f'final point mse = {final_point_mse[torch.argmin(path_mse)]}')
outputs = controller.run_control(y_ss=y_ss, R=R, num_steps=50)
est_targets = outputs['y_hat_ss']
estimated_y_seq = outputs['y_hat']
controlled_y_seq = outputs['y']
