...
if load_rnn:
    ...
    rnn.by.data = torch.zeros_like(rnn.by)
    rnn.by.requires_grad_(False)
    ...
    with torch.no_grad():
        ...
        if seed == 1:
            ...
            perturbed_rnn.W += 0.03*torch.randn_like(perturbed_rnn.W)
        else:
            perturbed_rnn.W += 0.01*torch.randn_like(perturbed_rnn.W)

        # for name, p in perturbed_rnn.named_parameters():
        #     if name == 'Wy':
        #         print('skip Wy')
