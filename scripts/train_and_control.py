...
x_ss_1 = torch.tensor([2.,]) #torch.linspace(-7, 7, 20)
x_ss_2 = torch.tensor([2.,]) #torch.linspace(-7, 7, 20)
...
for x_ss, (i,j) in zip(x_ss_list, idx_list):     
    print(f'Trial number {i}/{idx_list[-1,0]}, {j}/{idx_list[-1,1]}') 
    _, a_ss, y_ss = controller.generate_observation(x_ss)    
    outputs = controller.run_control(y_ss, x0=x0, num_steps=num_steps, Q=Q, R=R)
    outputs['x_ss'] = x_ss
    outputs['a_ss'] = a_ss

    err['x'][i,j] = (x_ss - outputs['x'][:,-11:-1,:].mean(dim=1)).norm()
    err['a'][i,j] = (a_ss - outputs['a'][:,-11:-1,:].mean(dim=1)).norm()    
    err['y'][i,j] = (y_ss - outputs['y'][:,-11:-1,:].mean(dim=1)).norm()
...
vmin = min(err['x'].min(), err['a'].min(), err['y'].min())
for var,ax in zip(['x', 'a', 'y'], axs):
    im = ax.pcolormesh(x_ss_1, x_ss_2, err[var], vmin=vmin, cmap='Reds')
    ax.axis('square')
    plt.colorbar(im, ax=ax)
    ax.set_title(f'{var} error')
    ax.set_xlabel('Target $x_{ss,0}$')
    ax.set_ylabel('Target $x_{ss,1}$')
fig.suptitle(f"$x_0=[{x0[0].item()},{x0[1].item()}]; Q={'' if Q==1 else f'{Q}\\times '}C^TC; R={'' if R==1 else f'{R}\\times '}I$")
fig.tight_layout()
fig.tight_layout()
