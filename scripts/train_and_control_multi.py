...
train_input = {'type': 'multi_level_noise', 'lo':-1,'hi':1,'step':0.1, 'probabilities':None, 'equal_prob':True, 'each_input_duration': 1}
...
config.model.save_dir = config.model.save_dir + (f"_u={'_'.join([str(train_input[key]) for key in train_input.keys()])}")
