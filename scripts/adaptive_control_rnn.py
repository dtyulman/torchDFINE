...
if VERBOSE:
    ...
    n_targets = 10
    fig, ax = rnn_train_data.plot_rnn_output(rnn, plot='z', n_targets=n_targets)
    fig, ax = rnn_train_data.plot_rnn_output(rnn, plot='h', h_dims=(4,7), n_targets=n_targets)
...
#         'model.save_steps': 500,

#         'train.plot_save_steps': 500,
#         'train.num_epochs': 1024*100,
#         'train.batch_size': 64,
...
controller = make_controller(trainer.dfine, clone_mats=False, **control_config)
closed_loop_train = make_closed_loop(plant=rnn, dfine=trainer.dfine, controller=controller, copy_dfine=False)

train_data = ControlledDFINEDataset(closed_loop_train, target_generator, num_steps=run_config['num_steps'])
...
try:
    ...
    while True:
        ...
        if control_config['u_min'] < control_config['u_max']:
            control_config['u_min'] += 1
            control_config['u_max'] -= 1
        else:
            break
...
#%% Actual target for z
# z_target = rnn_train_data.targets[:10] #[b,z]
...
x_hat_target = x_hat_target_proj_ctrb
closed_loop.run(x_hat_target=x_hat_target, aux_inputs=aux_inputs, **run_config)
...
for (key,val),ax in zip(errors.items(), axs):
    ax.hist(val[val<20], bins=20)
    ax.set_title(f'NMSE({key})')
fig.tight_layout()

#%%
fig, axs = closed_loop.model.plot_all(seq_num=0, max_plot_rows=7)
