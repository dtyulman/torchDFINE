...
import PSID
import sys
# sys.path.insert(1,'../PyPSIDDev/source')
# from DPAD.IPSID import PSID as IPSID
...
simulation_save_path = 'simulations/swissroll' + (f"_u={'_'.join([str(train_input[key]) for key in train_input.keys()])}")
...
if not use_saved_sim_data:
    ...
    simulation_dict = dict(zip(['x','a','y'],[x,a,y]))
...
fit_SID_parameters = True
load_model = False
# load_model = r"C:\Users\bilgi\Desktop\Research\torchDFINE\results\train_logs\2024-04-17\171753_u=binary_noise_-0.5_0.5_no_manifold"
# load_model = r"C:\Users\bilgi\Desktop\Research\torchDFINE\results\train_logs\2024-04-09\122258_u=binary_noise_-0.5_0.5"

config = get_default_config()
#################################################### THIS HAS TO BE SET TO TRUE HERE IF YOU ARE GOING TO USE SID PARAMETERS #############
...
config.model.save_dir = config.model.save_dir + (f"_u={'_'.join([str(train_input[key]) for key in train_input.keys()])}") + no_manifold_key
...
if not load_model:
    ...
    elif fit_SID_parameters:
        z = None # z is the behavior
        nx = config.model.dim_x
        n1 = 0 # n1 is the number of states that are going to be dedicated to behaviorally relevant dynamics, it should be 0 so that PSID becomes SID
        i = 10 #i is the subspace horizon used for modeling.
        idSys = PSID.IPSID(y.view(-1,dim_y).numpy(), z, u.view(-1,dim_u).numpy(), nx, n1, i)

        _,y_pred,_ = idSys.predict(y.view(-1,dim_y).numpy(),u.view(-1,dim_u).numpy())

        for neuron_num in range(dim_y):
            plt.plot(y[0,:,neuron_num])
            plt.plot(y_pred.reshape(num_seqs,num_steps,dim_y)[0,:,neuron_num])
            plt.show()
...
# pickle.dump(err['y'],open(config.model.save_dir + '\R_{}.pkl'.format(R),'wb'))
